...
if (typeof jQuery !== "function") {
    ...
    $.extend($.ig.loaderClass, {
        ...
        _initializePaths: function(type) {
            ...
            for (i = 0; i < len; i++) {
                ...
                for (j = 0; j < scriptData.length; j++) {
                    scriptData[j] = scriptData[j].replace("$localePath$", localePath).replace("$path$", path).replace("$theme$", theme)
                }
            }
        },
        _log: function(data) {
        ...
        _findWidgetScriptData: function(widgetName, parentWidget) {
            var i, len = this._resources.length,
                scriptData;
            if (widgetName.length > 3 && widgetName.indexOf(".js") === widgetName.length - 3) {
                return {
                    widget: widgetName,
                    scripts: [this.settings.scriptPath + "/" + widgetName],
                    css: []
                }
            }
            if (widgetName.length > 4 && widgetName.indexOf(".css") === widgetName.length - 4) {
                return {
                    widget: widgetName,
                    scripts: [],
                    css: [this.settings.cssPath + "/" + widgetName]
                }
            }
            for (i = 0; i < len; i++) {
                scriptData = this._resources[i];
                if (scriptData.widget === widgetName && (scriptData.parentWidget !== undefined && parentWidget !== undefined && scriptData.parentWidget.indexOf(parentWidget) > -1 || parentWidget === undefined || parentWidget === widgetName)) {
                    return scriptData
                }
            }
            return null
    ...
    $.ig._loadWorkItem.prototype = {
        ...
        _loadDependency: function(scriptData, loadingEntity, type) {
            var j, len = scriptData.dependency ? scriptData.dependency.length : 0,
                name;
            for (j = 0; j < len; j++) {
        ...
        _loadFeatures: function(type, widgetName) {
            var features, i, len, parentWidget;
        ...
        _loadAllFeatures: function(type) {
            var i, j, len = this._loader._resources.length,
                jlen = 0,
                scriptData, item, loadingEntity = "ALL",
                res;
            for (i = 0; i < len; i++) {
        ...
        loadWidgetResources: function(widgetName, callback) {
            if (this._loadingEntities[widgetName] !== undefined) {
                ...
                this._loadingEntities[widgetName] = {
                    name: widgetName,
                    call: [callback],
                    queue: []
                }
        ...
        loadAllResources: function(callback) {
            ...
            this._loadingEntities[loadingEntity] = {
                name: loadingEntity,
                call: [callback],
                queue: []
            };
